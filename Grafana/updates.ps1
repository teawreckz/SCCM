Function SCEP{
    ...
    SELECT TOP 5 [ep].[AntivirusSignatureVersion],
        ...
        into #temp_scep_$Coll_Type
            FROM [v_GS_AntimalwareHealthStatus] [ep]
                LEFT JOIN [CM_VIT].[dbo].[v_FullCollectionMembership] [vc] ON [vc].[ResourceID] = [ep].[ResourceID]
    ...
    "
    try {
        $result = Invoke-Sqlcmd $sqlCmd  -server ipvitcmssql001b.adm.datakraftverk.no -Database CM_VIT
    }
    catch {
        return $false
    }
...
Function Updates_Last{
    ...
    SELECT '$Filter_Name' AS [filter],
           ...
           FROM [grafana].[dbo].[$Coll_Type] as [s]
                LEFT JOIN [CM_VIT].[dbo].[v_FullCollectionMembership] [vc] ON [vc].[resourceID] = [s].[ResourceID]
           WHERE [vc].[CollectionID] = '$Coll_ID'

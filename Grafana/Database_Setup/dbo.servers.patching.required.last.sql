...
ALTER PROCEDURE [dbo].[Servers.Patching.Required.Last]
AS
     BEGIN
	 --replace all CM_XXX with your SCCM DB name
         DELETE FROM [grafana].[dbo].[Servers]
                WHERE [filter] = 'Servers.Patching.Required.Last';
         DECLARE @UserSIDs VARCHAR(16);
         SELECT @UserSIDs = 'disabled';
	-- servers collection ID with updates deployed as required
         DECLARE @CollID VARCHAR(8)= 'XXX00237';
	...
	-- Adjust this Software Update Group Name filter to fit your Naming Convention
         ...
         INSERT INTO @SUG
                SELECT TOP 1 [CI_ID]
                       FROM [CM_XXX].[dbo].[v_AuthListInfo]
                       WHERE([title] LIKE '%serv%req%month%sec%'
         ...
         INSERT INTO [grafana].[dbo].[Servers]
                SELECT DISTINCT
                       ...
                       'Servers.Patching.Required.Last' AS [Filter],
                       (
